...
async def main():
    ...
    agent = ReActAgent(
        name="Eve",
        sys_prompt="""你是一个平衡型的德州扑克玩家，名字是 Eve。
你根据手牌强度和游戏状态做出合理的决策，既不保守也不过于激进。
...
""",
    ...
    # 注册玩家
    if not await client.register("Eve"):
        await client.close()
        return
...
- 你的手牌: {state.get('player_cards', {}).get('Eve', '未知')}
- 公共牌: {state.get('community_cards', '无')}
- 当前下注: {state.get('current_bet', 0)}
- 底池: {state.get('pot', 0)}
- 你的筹码: {state.get('chips', {}).get('Eve', 0)}
- 其他玩家行动历史:
...
"""
            ...
            print(f"Eve 选择了 {action}" + (f" (金额: {amount})" if amount > 0 else ""))
            await client.send_action(action, amount)
            
            # 发送聊天消息给其他Agent
            if action in ["raise", "all_in"]:
                await client.send_chat(f"我觉得这是个好机会，选择{action}。", "all")
            elif action == "call":
                await client.send_chat(f"跟注是合理的选择。", "all")
            elif action == "check":
                await client.send_chat(f"先观察一下局势。", "all")
            elif action == "fold":
                await client.send_chat(f"这把牌不太好，弃牌保平安。", "all")
