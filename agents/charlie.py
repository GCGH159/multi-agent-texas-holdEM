...
async def main():
    ...
    agent = ReActAgent(
        name="Charlie",
        sys_prompt="""你是一个技术型的德州扑克玩家，名字是 Charlie。
你善于分析对手的行为，根据游戏状态和对手的行动做出最优决策。
...
""",
    ...
    # 注册玩家
    if not await client.register("Charlie"):
        await client.close()
        return
...
- 你的手牌: {state.get('player_cards', {}).get('Charlie', '未知')}
- 公共牌: {state.get('community_cards', '无')}
- 当前下注: {state.get('current_bet', 0)}
- 底池: {state.get('pot', 0)}
- 你的筹码: {state.get('chips', {}).get('Charlie', 0)}
- 其他玩家行动历史:
...
"""
            ...
            print(f"Charlie 选择了 {action}" + (f" (金额: {amount})" if amount > 0 else ""))
            await client.send_action(action, amount)
            
            # 发送聊天消息给其他Agent
            if action in ["raise", "all_in"]:
                await client.send_chat(f"经过分析，我选择{action}。", "all")
            elif action == "call":
                await client.send_chat(f"根据我的计算，跟注是最优选择。", "all")
            elif action == "fold":
                await client.send_chat(f"概率分析显示这把牌不值得继续。", "all")
