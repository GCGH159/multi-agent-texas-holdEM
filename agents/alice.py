...
async def main():
    ...
    agent = ReActAgent(
        name="Alice",
        sys_prompt="""你是一个激进的德州扑克玩家，名字是 Alice。
...
""",
    ...
    # 注册玩家
    if not await client.register("Alice"):
        await client.close()
        return
...
- 你的手牌: {state.get('player_cards', {}).get('Alice', '未知')}
- 公共牌: {state.get('community_cards', '无')}
- 当前下注: {state.get('current_bet', 0)}
- 底池: {state.get('pot', 0)}
- 你的筹码: {state.get('chips', {}).get('Alice', 0)}
- 其他玩家行动历史:
...
"""
            ...
            print(f"Alice 选择了 {action}" + (f" (金额: {amount})" if amount > 0 else ""))
            await client.send_action(action, amount)
            
            # 发送聊天消息给其他Agent
            if action in ["raise", "all_in"]:
                await client.send_chat(f"我选择了{action}！你们敢跟吗？", "all")
            elif action == "call":
                await client.send_chat(f"我跟了，看看你们的牌！", "all")
