...
async def main():
    ...
    agent = ReActAgent(
        name="Bob",
        sys_prompt="""你是一个保守的德州扑克玩家，名字是 Bob。
你需要根据游戏状态和对手的行为做出最优决策。
...
""",
    ...
    # 注册玩家
    if not await client.register("Bob"):
        await client.close()
        return
...
- 你的手牌: {state.get('player_cards', {}).get('Bob', '未知')}
- 公共牌: {state.get('community_cards', '无')}
- 当前下注: {state.get('current_bet', 0)}
- 底池: {state.get('pot', 0)}
- 你的筹码: {state.get('chips', {}).get('Bob', 0)}
- 其他玩家行动历史:
...
"""
            ...
            print(f"Bob 选择了 {action}" + (f" (金额: {amount})" if amount > 0 else ""))
            await client.send_action(action, amount)
            
            # 发送聊天消息给其他Agent
            if action == "fold":
                await client.send_chat(f"这把牌不好，我弃牌了。", "all")
            elif action == "check":
                await client.send_chat(f"我选择过牌，看看情况再说。", "all")
            elif action == "call":
                await client.send_chat(f"我跟了，稳扎稳打。", "all")
